...
| 20 | Plan the next wave on `gen_irac_analysis_v1,gen_irac_analysis_v3,gen_summarization_v2` | **OPEN, SIZED at ~780 synthesis tasks on v1+v3** (~1,279 at the pooled yield). Without it the drained queue lands 418 effective rows BELOW the band and the corpus cannot be assembled at all (F37, F41). **Do NOT dispatch yet:** the claim is FIFO by rowid, so a wave planned now is worked LAST and the dispatch costs up to ~4 h of idle fleet. Trigger is queue depth (< ~1,000 pending synthesis), not the clock (F43). **Command rehearsed and corrected (F45): `--n` counts the LIVE arm-NULL queue, so it is `<live> + 780` - the stream total over-plans 2.3x**. ETA now MEASURED, not guessed: the queue drains ~863 pending/run at ~315.6 min/cycle, so 4,072 pending is ~25 h and the whole queue is dry around 2026-09-01 14:00Z. The <~1,000-synthesis trigger arrives sooner - roughly two runs out, ~2026-09-01 00:00Z - but the per-stream split has to be read on the baton at dispatch time, not extrapolated (F50) **COMMAND SIMPLIFIED 2026-08-31 (F55): it is now `python -m tuned.data.tasks --config data/configs/data_law_v1.yaml --stream synthesis --add 780 --mix <task_type=weight,...> --variant gen_irac_analysis_v1,gen_irac_analysis_v3` - `--add` resolves the target against the live queue INSIDE the job, so there is no reading to take first and no stale sum to carry. That also removes the constraint that it be dispatched at the boundary right after a reading; it can go at any boundary. Still a dispatch, still displaceable by a cron fire, and the window is now the one after the `47 */4` fire rather than the `17 */4` one (F54).** |
| 21 | Verify the ceiling guard's first live run | **DONE 2026-08-31, verified against its pre-registration.** Run `33381288057` at 12:35:20.777Z: `ceiling guard: serving every stream - 503 effective generated-curated rows against a ceiling of 2050`. Registered form was `~398 effective ... ceiling of 2050`; the verdict, the phrasing and the ceiling all match, and 503 vs ~398 is drift from rows accepted between the estimate and the run start. As registered, what this confirms is that the guard MEASURES and SERVES, not that it throttles - 503 is far below the 1,900 throttle point that draining the queue would reach. Run ended **success** 17:50:39Z; final task states: accepted=1791, pending=3261, rejected=2647, format_parked=1227, stale_prompt=502, gen_unroutable=14. |
...
| 27 | **OPERATOR ACTION, DEADLINE ~2026-09-02: squash the baton's git history** | **OPEN, hard stop.** 55.00 GB of a 100 GB ACCOUNT-WIDE free private quota is already used (baton 38.83 + 16.17 in ten checkpoint repos) and it grows 14.8 GB/day - every checkpoint is a fresh ~565 MB LFS blob; wall ~2026-09-03, and a squash needs up to 36 h to reflect. `super_squash_history` takes it back to ~1 GB, is irreversible, and must run when NO worker holds the baton - end-of-job after the final push, or cancel-squash-dispatch by hand. Recurs ~weekly (F48). **EXACT FIGURES 2026-08-31 13:25Z** from HF `usedStorage` (not derived): baton 42.07 GB, seven checkpoint repos 16.16 GB, **total 58.23 GB of 100** - ~3.2 GB above what F48 recorded this morning, so the 14.8 GB/day on file is probably LOW and the wall probably EARLIER than 09-03. Rate now being measured over a timed interval rather than derived. **A squash is not the only lever**: six of the seven checkpoint repos are retired lanes holding 6.84 GB - `-rslora` 0.54 + `-rslora32` 1.59 + `-alpha64` 1.06 (arms retired in 15c3eb9 / 2b3ac29 / c3c3651, both questions recorded FULLY CLOSED) and `qwen-ckpt` 0.78 + `-ckpt-ddp` 1.30 + `-ckpt-manual` 1.57 (pre-8B lanes, superseded when the repo was stripped to Qwen3-8B DDP on 2026-08-08). Only `qwen8b-ckpt-ddp` (8.68 GiB) is live. That is ~half a squash's worth of headroom for a delete that needs no quiet window - but it is irreversible and may be the only copy of those weights, so it is an OPERATOR call, listed not recommended. **RATE NOW TIMED, AND IT IS 2.8x THE FIGURE ON FILE: 58.24 GB at 13:24:19Z -> 59.97 GB at 14:24:23Z = +1.73 GB in 1.00 h = 41.5 GB/day.** Headroom 40.0 GB, so the wall is **~2026-09-01 14:20Z, about 24 h out** - and a squash needs up to 36 h to reflect, so IT IS ALREADY LATE. Mechanism: the DB is NOT the main cost. `raw/gen/<day>/gen.ndjson` is append-only and ~250 MB by mid-day, and the 900 s cadence re-uploads it WHOLE as a fresh LFS blob 4x/hour = ~1.0 GB/h against the hourly DB's 0.565 GB/h. It grows all day, so the rate ACCELERATES until the UTC date rolls the file over (F52) **MITIGATED, NOT SOLVED, 2026-08-31 14:55Z (F53):** the raw log now shards by hour (`gen-<hour>.ndjson`), which the Hub skips once frozen - burn ~42.7 -> ~19.5 GB/day, wall **~09-01 12:00Z -> ~2026-09-02 10:00Z**. Verified by model closure first: the remote's working tree is 1.336 GB and only TWO files change between pushes (DB 749.2 MB x1/h + raw 257.6 MB x4/h = 1.78 GB/h vs 41.5 GB/day timed), so those two files are the whole burn. The squash is still required and still needs its 36 h, but it is no longer due inside the operator's night. Takes effect on the run starting ~17:50Z; run `33381288057` is stamped `c84fb1f` and burns at the old rate until ~17:49Z. After this the DB is ~95% of the remainder - compressing the snapshot (~4x, ~5 GB/day) and `--db-every` 3600->7200 are both priced in F53 and both deliberately NOT taken. **THE PERMANENT FIX IS NOW MEASURED, NOT GUESSED (F57):** `dbstat` says **93% of the database is the `seed` table** (526.4 MB of 565.2 MB on the 08-29 snapshot), and `seed` is written only by acquire/extract - the worker phase never touches it. So the hourly checkpoint re-uploads half a gigabyte of byte-identical legal source text. Splitting `seed` into its own attached file takes the burn from ~19.5 to ~6 GB/day and the squash from every ~5 days to every ~16. Smaller than it looks: SQLite resolves unqualified table names through attached databases, so no query changes - only `Store.open`, `snapshot_db` and `restore_bundle`. Deliberately a daylight change: it is the same restore path, and the same stale-file trap, that the compression lever was refused for. **CONFIRMED LIVE 18:12Z AND THE LEVER PULLED (F59):** the baton now carries `gen-17.ndjson` 2.7 MB and `gen-18.ndjson` 2.4 MB beside a frozen `gen.ndjson` 319.1 MB, so F53 works as designed (~30x off the raw term, measured). The worker's own countdown reads **`storage: 67.8 GB of 100 used, 32.2 GB headroom`**, which at the new 18.8 GB/day is a wall at **~2026-09-02 11:18Z** - and since a squash needs up to 36 h to reflect, the operator's real deadline was **~2026-08-31 23:18Z, inside their night**. So `--db-every` 3600->7200 WAS taken (it was priced and declined this morning; the trigger condition changed, not the reasoning): burn ~9.8 GB/day, wall **~2026-09-03 18:40Z**. Cost is a doubled crash-loss bound (~140 paid generations instead of ~70, only on a crash), which is strictly cheaper than a wall that costs every run its whole 5h15m. **PUT `--db-every` BACK TO 3600 ONCE THE SQUASH LANDS** - it is borrowed margin, not a new default. |
| 28 | Decide whether ~36 dual judgements/day is enough quality evidence | **OPEN, OPERATOR DECISION - the lever CHANGED, see F58.** Measured on run `33381288057`: 270 rows decided, 258 gate-accepted, 11 sampled-but-unjudged, and **1 real dual judgement for 77,461 tokens**. The ceiling is not groq's (live headers: 999/1000 requests, 7988/8000 tokens, per-minute refill) but **our own `tpd: 200000` per groq model** in `data_law_v1.yaml:555,572`, shared across runs via the baton ledger. A judge call is ~5.5k tokens - prompt 4,914-5,661, verdict **238** - so **~96% of the daily judge budget is spent shipping the row to the judge, not judging it**, and 200k/5.5k reproduces ~36 calls/model/day = ~36 dual judgements/day from measurement rather than assertion. **But the prompt is NOT padding** - measured, the rubric is only ~900 tokens (16-18%) and the rest is `{source}` + `{candidate_think}` + `{candidate_answer}`, each of which is the evidence for one of the three axes (`reasoning_validity` scores the trace step by step). Cutting it would delete an axis, not save postage. **So two exits are CLOSED: the sample size is already right at 0.02, and shrinking the prompt is not free.** What remains is (a) accept ~36/day, or (b) pair each groq ref with cerebras (784k left) instead of with the other groq ref, ~36 -> ~72/day - the only lever with real headroom, carrying the tiebreak-family objection. `tpd: 200000` is groq's real published limit, not a guessed floor to raise. Also settled: `openai/gpt-oss-20b` needs no `reasoning_effort` guard (238/1024 tokens, clean JSON; `'none'` is a 400 on it) - do not "fix" that block. |
...
### F59. F53 CONFIRMED LIVE, AND THE EMERGENCY LEVER PULLED BEHIND IT

**F53 verified on the baton at 18:12Z**, not inferred:

    raw/gen/2026-08-31/gen.ndjson      319.1 MB   <- frozen, no longer growing
    raw/gen/2026-08-31/gen-17.ndjson     2.7 MB   <- hourly shards, same depth
    raw/gen/2026-08-31/gen-18.ndjson     2.4 MB

A full hour's shard lands ~12-15 MB, so the re-uploaded raw term is ~35 MB/h
against the ~1,030 MB/h it replaced - the predicted ~30x cut, measured rather
than modelled. The storage countdown shipped this morning also spoke for itself
on its first run: `storage: 67.8 GB of 100 used, 32.2 GB headroom`.

**And that reading is what forced the next decision.** With the raw term gone
the DB is ~95% of what is left, so the burn is 0.749 + 0.035 = 0.784 GB/h =
**18.8 GB/day**, and 32.2 GB of headroom is a wall at **~2026-09-02 11:18Z**.
`super_squash_history` remains the real fix and reclaims far more - but the
quota takes **up to 36 h to reflect** one, which puts the operator's true
deadline at **~2026-08-31 23:18Z**, in the middle of their night. A squash on
09-01 would reflect AFTER the wall.

**So I pulled the lever I had priced and declined this morning: `--db-every`
3600 -> 7200.** The trigger condition changed, not the reasoning. Burn goes to
~9.8 GB/day and the wall to **~2026-09-03 18:40Z**, buying the squash a waking
day to happen in.

**What it costs, stated as a durability bound rather than a tuning knob:**
`--db-every` IS the crash-loss bound - `reconcile_raw` replays generations and
judgements from the raw log but neither task state nor `gate_result`, so a crash
now returns up to TWO hours of paid generation to `pending` instead of one
(~140 generations at the observed ~72/h). It is paid ONLY on a crash; the
end-of-job push always carries a database and the last run exited `success`.
...
worker never writes it, so splitting it out gives ~6 GB/day and makes both this
lever and the weekly squash unnecessary.
...
**SELF-CORRECTION, before this reached the operator: "halve the prompt" is not
a free lever, and I checked instead of writing it down.** Composition measured:
`judge_pointwise_v1.md` is 3,600 chars ~= **900 tokens, only 16-18% of the
prompt**, so compressing the rubric buys almost nothing. The other 82-84%
(~4,000-4,760 tokens) is the three substituted fields - `{source}`,
`{candidate_think}`, `{candidate_answer}` - and reading the rubric shows **none
of them is padding**: `grounding_faithfulness` scores every proposition against
`{source}`, `reasoning_validity` scores whether each step of `{candidate_think}`
follows, and the rubric explicitly instructs that a trace which "hesitates,
doubles back, corrects itself or admits uncertainty" is doing its job. Dropping
the trace to save ~4k tokens/call would not shrink the prompt, it would **delete
one of the three axes** and silently redefine the quality bar. So the postage
...
**Sharper still: groq was at `left=0k` in this run's FIRST gen batch** (12:41:35Z,
`spent=200.6k` / `205.2k`). The budget was gone before the run generated a single
row - spent by its predecessors - so the whole 5h15m had no dual-judgement
capacity at all beyond the one judgement that landed in the opening minutes.
This is not a run that exhausted its budget; it is a run that inherited an empty
one. Whatever the daily ceiling is, the FIRST run after the UTC reset consumes
it and every later run of that day judges nothing. That is an argument about
*distribution* the 0.02 sample does help with, and it is the one part of F56's
original reasoning that survives intact.
...
**What that leaves for task 28**, honestly narrowed rather than widened:
  ...
  * change the fleet topology - pair each groq ref with cerebras (784k budget
    left) instead of with the other groq ref, ~36 -> ~72/day. This is the only
    lever with real headroom, and it carries the tiebreak-family objection
    already recorded, so it is an operator call and not an overnight one.
  * `tpd: 200000` is NOT a guessed floor to be raised - it is groq's published
    per-model daily token limit (recorded 2026-08-19), so the ceiling is real.
The sample size is already right and the prompt is not padding; those two exits
are now closed rather than left open as things that might work.
...
that goes. `dbstat` over the local 2026-08-29 snapshot (565.2 MB, read-only,
not the baton):

    seed            526.4 MB      93.1%
    generation       13.4 MB
    document          6.0 MB
    seed_by_source    4.2 MB
    chunk_manifest    2.6 MB
    artifact          2.2 MB
    run_event         2.1 MB       <- not the diagnostics, as it turns out
    task              1.7 MB
    gate_result       1.7 MB

**And `seed` does not change during a worker run.** The only writes to it are
`INSERT OR REPLACE` at ingest and the two `DELETE FROM seed` re-draw paths
(`store.py:842`, `store.py:885`), all of which belong to the acquire/extract
phases. The worker phase runs generate and judge, and neither touches it. So
every hourly checkpoint re-uploads half a gigabyte of legal source text that
has been byte-identical since the seeds were ingested - and it has to, because
LFS versions whole files and `snapshot_db` VACUUMs the file, so the bytes
differ everywhere even when the content does not.

This is F53's lever again, on the bigger term: an unchanged file is free, so
the fix is to stop the immutable part from sharing a file with the mutable
part. Today's DB is 749 MB against that snapshot's 565, and the growth is
`generation` (13.4 MB then), so seed is now ~70% of it: splitting it out takes
the DB term from ~749 MB/h to ~220 MB/h, and the whole burn from ~19.5 to
**~6 GB/day**. The squash goes from a chore every ~5 days to one every ~16,
and it keeps improving relative to a growing corpus rather than degrading.

**The fix is smaller than it looks, and this is the part worth writing down.**
The obvious objection is that moving a table to its own file means qualifying
every query that reads it. It does not: SQLite resolves an unqualified table
name by searching `temp`, then `main`, then attached databases in attach
order. If `seed` exists ONLY in an attached `state/seed.sqlite3`, every
existing `... FROM seed` keeps working untouched. What actually has to change
is `Store.open` (ATTACH), `snapshot_db` (write two files), and
`restore_bundle` - which is exactly the crown-jewel path F53 already refuses
to touch unattended, and for the same reason: it carries the same stale-file
trap, because `Bundle.push` passes no `delete_patterns` and the old
single-file DB would sit on the remote forever beside the new pair.

**NOT built tonight.** It is the right permanent fix for task 27 and it is a
daylight change with a live verification window, not an overnight one. What is
recorded here is the measurement, the mechanism and the design, so the next
...
than inferred, and the mechanism is confirmed with no free parameters left.

**Near miss, recorded so it is not repeated.** `[audit-accept=N]` in a judge
batch line is NOT a dual judgement - it is `audit:gate-accept`, the 95% the
hash did NOT select, accepted on gates alone. Reading it as "judged" gives 323
judged rows in that run and a completely wrong picture. Worse, the run's own
summary line invites the same error: `dual-judged sample: 112 decided, 71
accepted -> accept rate 63.4%` is CUMULATIVE over every judgement in the store
("all recorded judgements (no --since given)", printed right above it), so it
pools the pre-audit dual era with the audit sample and cannot answer a
question about either. The real per-run figures come from counting `accept=`
and `reject=` tags that are not `audit-accept=`.

**What the run actually did:**

    decided                393
      audit gate-accept    373    the hash did not select these
      dual-judged accept    15    3.8% against --audit-sample 0.05
      dual-judged reject     0
      sampled, no judge      5    shipped audit:gate-accept:unjudged

So F51's "15 judged rows a run, 0 rejections" is CONFIRMED, and 20 sampled of
393 is 5.1% - the hash is doing exactly what it was told.

**The ceiling is a DAILY budget, and that is the new fact.** The run's earliest
visible reading is already `groq/qwen spent=117.4k left=83k` - inherited from
the runs before it on the same UTC day - and it ends `left=0k` on BOTH groq
refs while `cerebras left=784k` and `mistral left=5000k` sit idle. 400k of
groq at ~11k per dual judgement is ~36 judgements a day, which is exactly the
"~35-40 rows/UTC-day" the audit-mode comment was written around.

**The truncation is visible to the minute.** Last dual judgement 11:30:25;
every sampled row after it - 11:40, 11:51, 12:15, 12:28 - shipped unjudged.
Four consecutive misses after fifteen consecutive hits is not sampling noise.

**So 0.05 was never sized against the fleet.** ~1,800 rows decided a day x
0.05 = ~90 selected against ~36 servable: ~54 a day ship unjudged, and the 36
that ARE judged are all from before the budget dies. They are the early part
of a FIFO queue, not the uniform sample `audit_sampled`'s docstring promises -
the same bias the `AUDIT_UNJUDGED_DISPOSITION` comment already names for a
different cause. **Re-sized to 0.02**, which buys the same ~36 judgements
spread over the whole day, for nothing. The test asserts the RELATIONSHIP
(selected <= servable, and >= 75% of it) rather than the number, so the rate
cannot be left behind if the ceiling moves.

**A hypothesis worth testing, NOT acted on: the ceiling may be doublable for
free.** Both groq refs end the day fully spent (200.6k and 205.2k) while
cerebras has 784k left. If slots A and B are being filled with `groq/qwen` and
`groq/openai/gpt-oss` - two distinct families, both first in `routing.judge` -
then each judgement burns ~5.5k from each groq ref. Pairing each groq ref with
cerebras instead would burn one groq ref per judgement and put the other on
cerebras' idle 784k, giving ~72 a day instead of ~36. The measurement that
settles it is a per-ref judgement count, which the batch lines do not carry
(they report fleet-wide spend, from the GEN log). **Not attempted:** it is a
change to the judging topology, and the reason F51 gave for leaving mistral
alone applies here too - a contested row needs a THIRD distinct family for the
tiebreak, and after groq dies a mistral+cerebras pair would have none.

Task 28 is unchanged and still the operator's: whether ~36 dual judgements a
day, now unbiased across the day, is enough evidence to ship on. What has
changed is that the number is no longer 15-with-an-asterisk. One test. Suite
**3,840 / 19** (+1).

### F55. THE WAVE COMMAND NO LONGER CARRIES A STALE NUMBER - `--add` SHIPPED

Task 20's command was `--n <live> + 780`, and the arithmetic was the problem.
`--n` is a TARGET measured against the live arm-NULL queue (F45), the planner
can only run at a run boundary, and the live count has to come from the
boundary BEFORE it - which is 5.25 h and ~863 drained rows earlier. Passing a
sum computed then means the planner tops up to a target that was right hours
ago and the wave lands several hundred rows too big.

**That error is one-way**, which is why it is worth a code change rather than
care at dispatch time. A short wave can be topped up on the next boundary; an
over-planned one cannot be undone, because generated rows cannot be dropped
from the corpus afterwards and the band has a high side (F41, and the same
one-way-door argument as the curated ceiling in F35).

`--add N` resolves `args.n = _existing_in_queue(...) + N` inside the job,
against the queue that is actually there. Refused alongside `--n`: "make it
this big" and "make it this much bigger" are two answers to one question.
Resolved before the re-open, and the order is immaterial rather than
convenient - `_existing_in_queue` already counts the recoverable parking
states as live, so moving rows out of them does not change the number.

**Task 20's command is now literally `--add 780`** with no reading to take
first, which also means it no longer has to be dispatched at the boundary
immediately after the reading. Three tests. Note one of them was written
wrong first and caught: `pytest.raises(SystemExit)` on `--n 5 --add 3` passed
BEFORE the flag existed, because argparse exits on an unrecognised argument -
so it asserts the message and that it does not say "unrecognized". Suite
**3,839 / 19** (+3).
...
operator, no destructive step and no durability trade.

**The model, now closed against the measurement.** F52 reasoned from cadence;
this reads the remote's working tree, so the two can be reconciled file by file:

    state/law_v1.sqlite3                749.2 MB   x1/h  = 0.749 GB/h
    raw/gen/2026-08-31/gen.ndjson       257.6 MB   x4/h  = 1.030 GB/h
    every other file (unchanged bytes)                     ~0
    ------------------------------------------------------------
                                                   1.78 GB/h = 42.7 GB/day

against 41.5 GB/day timed. The model is closed: those two files ARE the burn.
Note the DB is 749 MB, not the 565 MB on file - it has grown - and that the
other 26 files cost nothing, because the Hub already holds those bytes and
skips them. **That is the whole lever**: an unchanged file is free, so the
problem is not what the bundle contains but that one big file changes every
time.

**The change.** `raw_gen_path` now returns `raw/gen/<day>/gen-<hour>.ndjson`.
One re-upload is capped at an hour of generation (~18 MB at the measured 17.6
MB/h), and every earlier shard freezes and is skipped. Raw drops from ~1.03
GB/h to ~0.035 GB/h - a factor of ~30 - with the same data, the same 900 s
cadence, and the same durability rule: the raw write is still the first thing
that happens to an answer somebody paid for.

**Why it is safe, stated as an invariant rather than a hope.** The shard sits
at the SAME DEPTH as the file it replaces. `reconcile.default_raw_paths` globs
`raw/<kind>/*/*.ndjson` - exactly one directory level - so both layouts are
swept by the same pattern, and every recovered row is pointed at the file it
actually occupies rather than at a name computed later. Logs written before
today keep resolving unchanged. That invariant is asserted by a test, not
assumed, because getting it wrong would silently strand crash recovery, which
is the one thing the raw log exists for.

**Effect, and it is honest about what it does not do.** ~42.7 -> ~19.5 GB/day.
That does not remove the wall, it moves it: with 33 GB of headroom expected at
17:50Z, ~09-01 12:00Z becomes **~2026-09-02 10:00Z**. A squash still has to
happen and still needs its 36 h, but the deadline is no longer inside the
operator's night.

**Timing.** Run `33381288057` is stamped `c84fb1f`, which predates this, so it
burns at the old rate until it ends ~17:49Z. The cron is `17 */4 * * *`, so the
16:17Z fire queues behind it and starts ~17:50Z with the fix. Verification is a
rate re-measure after that, not an inspection of the diff.

**What is deliberately NOT done, priced so it need not be re-derived.** After
this, the DB is ~95% of what is left. Two levers exist and neither was taken:

- **Compress the snapshot.** A VACUUMed SQLite of mostly text compresses ~4x,
  so 749 -> ~190 MB and the burn goes to ~5 GB/day. Zero cadence or content
...
- **`--db-every` 3600 -> 7200.** Halves the DB term, one reversible flag. NOT
  ...
  was refuted three times because rows whose answers were already bought come
