...
| 28 | Decide whether 15 dual-judged rows/run is enough quality evidence | **OPEN, OPERATOR DECISION** - the audit sample is sized right (5.09%) but delivers 15 judged rows and 0 rejections per run, with 25% of the sample shipping unjudged. Cause is family exhaustion, not budget: after groq's two models hit their daily cap there is only ONE free family left for a deepseek row and a dual judgement needs two. mistral's idle 5,000k is NOT the fix - it is the reserved tiebreak family (F51) **RE-MEASURED 2026-08-31 (F56):** the mechanism is confirmed to the minute (groq `left=0k` on both refs, last dual judgement 11:30:25, then four consecutive sampled-but-unjudged rows) and the budget is DAILY and shared across runs, not per-run. `--audit-sample` re-sized 0.05 -> **0.02** because 0.05 selected ~90 rows/day against a ~36/day fleet: same evidence, spread across the whole day instead of truncated at 11:30Z. Also corrected: `[audit-accept=N]` is the GATE-ONLY path, not a dual judgement, and the run's `dual-judged sample: 112 decided ... 63.4%` line is cumulative over the whole store (pre-audit era included) so it answers neither question. The open lever for actually RAISING the ceiling is pairing each groq ref with cerebras rather than with each other (~36 -> ~72/day), which is a topology change and carries the same tiebreak-family objection as mistral. |
