...
| 28 | Decide whether ~36 dual judgements/day is enough quality evidence | **OPEN, OPERATOR DECISION - the lever CHANGED, see F58.** Measured on run `33381288057`: 270 rows decided, 258 gate-accepted, 11 sampled-but-unjudged, and **1 real dual judgement for 77,461 tokens**. The ceiling is not groq's (live headers: 999/1000 requests, 7988/8000 tokens, per-minute refill) but **our own `tpd: 200000` per groq model** in `data_law_v1.yaml:555,572`, shared across runs via the baton ledger. A judge call is ~5.5k tokens - prompt 4,914-5,661, verdict **238** - so **~96% of the daily judge budget is spent shipping the row to the judge, not judging it**, and 200k/5.5k reproduces ~36 calls/model/day = ~36 dual judgements/day from measurement rather than assertion. **So the first lever to price is the ~5,000-token judge prompt, NOT the sample size (already right at 0.02) and NOT the fleet topology (which carries the tiebreak-family objection).** Halving the prompt roughly doubles the rate for free. Not taken tonight: `task_id_for` does not hash `prompt_sha`, so a template edit parks every pending task `stale_prompt` permanently - this is a between-waves change, never an overnight one. Also settled: `openai/gpt-oss-20b` needs no `reasoning_effort` guard (238/1024 tokens, clean JSON; `'none'` is a 400 on it) - do not "fix" that block. |
