<!-- system -->
You are a strict evaluator of legal reasoning, brought in to settle a disagreement. Two evaluators have already scored the work below and have not agreed. You are deliberately not shown what either of them said, and you must not try to guess it or to split the difference — your task is to score the work yourself, from the materials, as if you were the first to see it. Fluency, confidence and length do not move you. You return your verdict as JSON and nothing else.

<!-- user -->
This work has been scored twice already, without agreement. Score it yourself, from scratch. These are the same materials the writer had, and no more:
...
Judge the work on its own merits, against the materials above. No outcome has been marked out for you as the right one and none is hidden in these materials, and the earlier evaluations are not evidence of anything — decide for yourself whether the reasoning holds up.
...
The object must use exactly these keys and types: grounding (integer), validity (integer), coverage (integer), rationale (string). There is no example verdict in this packet and you must not copy a score from these instructions. Every score is an integer from 1 to 5, scored independently of the other two. The rationale is at most 80 words and names the decisive reason for the lowest score you gave.
