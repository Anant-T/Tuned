...
# 2  the manifest records `teacher_mixture` - which teachers, at which prompt
#    generations, wrote the rows that actually shipped. Read off `_prov` on
#    the uploaded rows, so it counts the cut after dedupe and split rather
#    than the pool at accept time. A v1 manifest is silent about its teachers,
#    which is not the same fact as a v2 manifest reporting `not-generated`.
#    NOTE the version alone does not reach the Hub: idempotence is measured on
#    corpus + card bytes (same_uploaded_bytes), so a repo holding a v1 manifest
#    over an unchanged corpus stays v1 until the rows themselves change.
PUSH_VERSION = 2
...
def module_versions(out_dir: Path, report: dict) -> dict[str, int | str | None]:
    ...
    Row builders do not agree on one `_prov` shape either - only
    `decontaminate.generated_rows` writes `teacher`, because only its rows
    have one - but NONE of them writes extract_version, so a push.py that
    claimed one would be inventing it regardless.
...
def license_rows(report: dict) -> list[tuple[str, int]]:
    ...
    table on the same card. A row can still arrive unlicensed - a seed whose
    source carries no terms - which is exactly what `require_license: false`
    permits and what this row reports rather than drops.
    """
...
def teacher_mixture(rows: Sequence[dict]) -> dict[str, int]:
    """Which teachers, at which prompt generations, wrote the shipped rows.

    MEASURED OFF THE ROWS, not off the store, because the shipped set is not
    the accepted set: dedupe drops duplicate clusters and split holds rows
    back for eval, so a census taken where the rows were accepted counts
    teachers for rows that never shipped.

    A row with no `teacher` was not generated by one - replay copies and
    curated rows are carried in whole - and is counted under `not-generated`,
    which is a fact about the mix rather than a hole in it.

    Flat string keys so the manifest stays JSON, and so a reader can see at a
    glance whether a cut is one teacher at one template generation or a blend
    of several. verify.py's --require-generator decides who is ALLOWED in;
    this records who actually got in.
    """
    from collections import Counter
    ...
    for row in rows:
        prov = row.get("_prov") or {}
        teacher = str(prov.get("teacher") or "not-generated")
        counts[f"{teacher} @ {prov.get('prompt_sha') or '-'}"] += 1
...
def build_manifest(
    *, report: dict, decon: dict | None, push_cfg, versions: dict, outputs: Sequence[dict],
    chain: dict | None, rows: Sequence[dict] = (),
) -> dict:
    ...
    return {
        ...
        "tokenizer": report.get("tokenizer"),
        # WHO TAUGHT THIS CORPUS. Nothing else in the manifest chain carries
        # a provider or a prompt_sha forward - decontamination.json, dedupe,
        # split, assemble and stats all key on stream and state - so without
        # this a cut built from two teachers at three template generations
        # ships looking exactly like one built by the teacher configured
        # today. No cut can leave the machine without recording it.
        "teacher_mixture": teacher_mixture(rows),
