...
#
# off_teacher is the third and is here because NOTHING ELSE recovers it. The
# 2026-08-30 one-teacher cut demotes a row generated by a retired provider,
# and tasks.py lists verify:off-teacher in FREE_PARK_DISPOSITIONS precisely so
# the attempts come back - the row has to be bought again from the current
# teacher. But off_teacher is not in CLAIMABLE_STATES, so the queue reads
# empty with those rows sitting in it; and it is NOT in TERMINALLY_DEAD, so
# _existing_in_queue counts them as live and a re-plan at the same
# coordinates reports "already at target" and inserts nothing (task_id_for
# makes it an INSERT OR IGNORE). Without this entry the demotion is a
# one-way door: P1.1 says those rows "must regenerate" and none of them ever
# would.
#
...
REOPEN_ON_EMPTY = ("gen_unroutable", "format_parked", "off_teacher")
    ...
    if counts is not None and not servable:
        ...
        if not _claimable_in(root / DB_RELPATH, STREAMS):
            _report([
                ...
                "children. Plan a LARGER wave - `python -m tuned.data.tasks "
                "--config <cfg> --stream <stream> --n <target> --mix "
                "<task_type=weight,...>` - where <target> exceeds what the "
                "queue already holds: --n is a target for the whole stream, not "
                "an increment, so re-running the number that built the current "
                "queue reports 'already at target' and inserts nothing. Pass "
                "--mix explicitly; statute_qa has no eligible seeds and "
                "silently under-fills a default mix. Until then every run of "
                "this workflow will end here in ~3 min.",
            ])
